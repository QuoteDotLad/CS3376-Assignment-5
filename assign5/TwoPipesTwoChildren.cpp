...
//Description : Uses one parent and two children to execute commands
...
int main(int argc, char **argv){
 ...
 if((childOnepid = fork()) == -1){
   ...
   if (childOnepid == 0) { //First child
      ...
      close(pipeOne[0]);
      dup2(pipeOne[1],1); //Redirect standardoutput to pipeOne's write end
      close(pipeOne[1]);
...
if((childTwopid = fork()) == -1){
   ...
   else if(childTwopid == 0){ //Second child
   	   close(pipeOne[1]);
	...
	close(pipeOne[0]);
	close(pipeTwo[1]);
		dup2(pipeTwo[0],0); //Redirecting standard input to pipeTwo's read end
	close(pipeTwo[0]);
